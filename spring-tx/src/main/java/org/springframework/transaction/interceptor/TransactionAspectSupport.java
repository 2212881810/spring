...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			// 创建事务信息，（准备工作），核心方法。 在该方法内部已经创建好了conn ,创建好了事务信息，并且设置了事务信息，如隔离级别啥的
			...
			// 成功后提交事务 ，会进行资源释放，连接释放， 恢复挂起的事务操作等
	...
	protected final class TransactionInfo {
		...
		private void bindToThread() {
			...
			//将已经存在的事务信息保存起来，后面再用
			// methodA是required级别事务,methodB是 required_new级别的事务
			// 将methodA调用methodB方法时，methodA的事务会挂起，等methodB的事务执行完毕之后，再恢复methodA的事务
